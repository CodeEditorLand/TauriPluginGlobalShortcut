...
impl<R: Runtime> GlobalShortcut<R> {
	fn register_internal<F: Fn(&AppHandle<R>, &Shortcut, ShortcutEvent) + Send + Sync + 'static>(
		...
		self.shortcuts.lock().unwrap().insert(id, RegisteredShortcut { shortcut, handler });
		Ok(())
	}
...
impl<R: Runtime> GlobalShortcut<R> {
	...
	pub fn unregister_all(&self) -> Result<()> {
		...
		self.manager.unregister_all(hotkeys.as_slice()).map_err(Into::into)
	}
	...
	pub fn build(self) -> TauriPlugin<R> {
		...
		PluginBuilder::new("global-shortcut")
			...
			.setup(move |app, _api| {
				...
				app.manage(GlobalShortcut { app: app.clone(), manager, shortcuts });
				Ok(())
			})
			.build()
	}
